...
function getColorData() {
    ...
    for (var i = 0; i < textNodeLen; ++i) {
        ...
        if (textColor != "") {
			...
			for (bgNode = node; getComputedStyle(bgNode).getPropertyValue("background-color") == "transparent"; bgNode = bgNode.parentElement) if (!bgNode.parentElement) break;
			textNodeBgs.push(bgNode!=null ? getComputedStyle(bgNode).getPropertyValue("background-color") : "white");
        };
    };
